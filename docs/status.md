...
- 已完成：同文件顶层字符串常量动态导入支持。
- 已完成：`from pkg import name` 判定增强（结合 AST 顶层符号，减少误判子模块依赖）。
- 已完成：额外本地源码根支持（`--module-root`）和显式本地 include（`--include-module`、`--include-package`）。
...
- 已完成：CLI `--report` 构建报告（被打包模块、跳过模块、原因、依赖摘要、module roots、include modules/packages、module origins、risk level）。
- 当前失败点：无。
- 下一步：补文档与回归测试。
- 风险：更复杂动态行为仍会被拒绝。
...
- 已完成：新增回归覆盖（循环导入、import-time side effects、深层相对导入、脱离工程目录运行 bundled 文件、额外 module root、include 精确模块、include 包树、module root 歧义）。
- 当前失败点：无。
...
5. 决策：用 `--module-root` 声明额外本地源码根，用 `--include-module` 精确包含模块，用 `--include-package` 包含整个包树。
- 原因：避免从 `sys.path` 或 site-packages 猜测依赖，保持用户代码边界明确。
- 替代方案：按包名从当前 Python 环境解析。
- 风险：用户需要显式传入源码根；同名模块在多个 root 下会构建失败。

## C. 已知限制
- 仅支持常量字符串动态导入，以及同文件顶层单次赋值字符串常量；其他非常量或相对动态导入不支持。
